...
<div class="api-feedback">
	...
	?>
	<form class="form-horizontal" role="form" action="<?= $APPLICATION->GetCurPage() ?>" method="POST" 
		enctype="multipart/form-data" name="api_feedback_form" id="<?= $arParams['UNIQUE_FORM_ID']; ?>">
		<?= bitrix_sessid_post() ?>
		...
		if(count($arParams['DISPLAY_FIELDS']) > 0)
		{
			foreach($arParams['DISPLAY_FIELDS'] as $FIELD)
			{
				...
				<div class="form-group">
					<label class="col-md-3 control-label" for="<?=ToLower($FIELD)?>"><?= $field_name ?>:
						<? if(empty($arParams["REQUIRED_FIELDS"]) || in_array($FIELD, $arParams["REQUIRED_FIELDS"])): ?>
							<span class="asterisk"> *</span>
						<? endif ?>
					</label>
					<div class="col-md-9">
						<input class="form-control" id="<?=ToLower($FIELD)?>" type="text" value="<?= $arResult[$FIELD] ?>" name="<?= ToLower($FIELD) ?>"
		...
		if(count($arParams['DISPLAY_FIELDS']) > 0)
		{
			foreach($arParams['DISPLAY_FIELDS'] as $FIELD)
			{
				...
				{?>
				<div class="form-group">
					<label for="<?= ToLower($FIELD)?>" class="col-md-3 control-label"><?= $field_name ?>:
						<? if(empty($arParams["REQUIRED_FIELDS"]) || in_array($FIELD, $arParams["REQUIRED_FIELDS"])): ?>
							<span class="asterisk"> *</span>
						<? endif ?>
					</label>
		          ...
		          id="<?= ToLower($FIELD) ?>" name="<?= ToLower($FIELD) ?>" rows="5" 
							<?if($arParams['INCLUDE_PLACEHOLDER']):?> placeholder="<?= $field_name ?>" <?endif?>
		...
		<? endif; ?>
		<div class="api-submit pull-right">
			<button type="submit" name="submit" class="btn btn-primary" value="submit"><?=$arResult['FORM_SUBMIT_VALUE']?></button><br><br>
		</div>
	</form>
</div>