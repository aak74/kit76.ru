...
class CAkopObject {
  ...
  public function getList($arOrder = false, $arFilter = false, $arGroupBy = false, $arNavStartParams = false, $arSelectFields = false) {
	...
	public function update($id, $params) {
		...
		// 		$this->id = $id;
		// 	}
		// CAkop::pr_var($this->iblock, "iblock id");
		// CAkop::pr_var($id, "update id");
		// CAkop::pr_var($params, "update params");
		if (is_array($params)) {
			$this->id = $id;
			// echo 1;
			$el = new CIBlockElement;
			...
			if (isset($params["PROPERTY_VALUES"])) {
				// echo 2;
				$el->SetPropertyValuesEx($this->id, $this->iblock, $params["PROPERTY_VALUES"]);
